...
- (void) onExit  
{  
    [[CCEventDispatcher sharedDispatcher] removeDelegate:self];  
    [super onExit];  
}  
...
{  
	...
	//CGPoint touchPoint = [event locationInView:[touch view]];
...
{  
	CGPoint touchPoint = ccp([event locationInWindow].x,[event locationInWindow].y);
	touchPoint = [[CCDirector sharedDirector] convertToGL:touchPoint];
	if (ActiveRadius>0)
	{
