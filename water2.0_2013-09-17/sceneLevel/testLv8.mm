...
//    [self addSingleMap];
    ...
    // Particles
    initFluidPraticles(0.9f,0.010f,450.f,30.f,2000,@"fire_010.png",nil);// VbmyR.png fire2 drop2
...
//    touch1 = body2;
    ...
    bodyDef.position.Set(880.f/PTM_RATIO, 60.f/PTM_RATIO);
    tooth1 = m_world->CreateBody(&bodyDef);
    // Define another box shape for our dynamic body.
    b2PolygonShape dynamicBox;
    dynamicBox.SetAsBox(0.5f,52.f/PTM_RATIO);
    // Define the dynamic body fixture.
    b2FixtureDef fixtureDef;
    fixtureDef.shape = &dynamicBox;
    fixtureDef.density = 1.f;
    fixtureDef.friction = 0.0f;
    tooth1->CreateFixture(&fixtureDef);
    
    b2BodyDef bodyDef2;
    bodyDef2.type = b2_staticBody;
    bodyDef2.position.Set(880.f/PTM_RATIO, 166.f/PTM_RATIO);
    tooth2 = m_world->CreateBody(&bodyDef2);
    // Define another box shape for our dynamic body.
    b2PolygonShape dynamicBox2;
    dynamicBox2.SetAsBox(0.5f,52.f/PTM_RATIO);
    // Define the dynamic body fixture.
    b2FixtureDef fixtureDef2;
    fixtureDef2.shape = &dynamicBox2;
    fixtureDef2.density = 1.f;
    fixtureDef2.friction = 0.0f;
    tooth2->CreateFixture(&fixtureDef2);
...
-(void)creatRope
{
    ...
    anchorBodyDef.position.Set(screenSize.width/PTM_RATIO/2,(screenSize.height+100)/PTM_RATIO); //center body on screen
    ...
    bodyDef.position.Set(screenSize.width/PTM_RATIO/2, (screenSize.height)/2/PTM_RATIO);
    bodyDef.angle = 0.f;
    b2Body *body = m_world->CreateBody(&bodyDef);
    ...
    fixtureDef.friction = 0.0f;
    body->CreateFixture(&fixtureDef);
    body->ApplyForceToCenter(b2Vec2(600.f,0.f));
    tieChuiBody = body;
...
{
    if(body->toolsType == 1001)//移动势头的按钮
    {
    ...
    if(body->toolsType == 1002)//铁锤
    {
        if(fabs(body->GetAngularVelocity()) > 2.f)
        {
            body->SetAngularVelocity(fabs(body->GetAngularVelocity())/body->GetAngularVelocity()*2.f);
        }
    }
...
-(void)addMap
{
    ...
    verts[0].Set((640+632.0f) / PTM_RATIO, (360+353.0f) / PTM_RATIO);
    verts[1].Set((640+262.0f) / PTM_RATIO, (360+332.0f) / PTM_RATIO);
    verts[2].Set((640+292.0f) / PTM_RATIO, (360+141.0f) / PTM_RATIO);
    verts[3].Set((640+209.0f) / PTM_RATIO, (360+28.0f) / PTM_RATIO);
    verts[4].Set((640+201.0f) / PTM_RATIO, (360-147.0f) / PTM_RATIO);
    verts[5].Set((640+600.0f) / PTM_RATIO, (360-98.0f) / PTM_RATIO);
    verts[6].Set((640+593.0f) / PTM_RATIO, (360-342.0f) / PTM_RATIO);
    verts[7].Set((640+194.0f) / PTM_RATIO, (360-349.0f) / PTM_RATIO);
    verts[8].Set((640+196.0f) / PTM_RATIO, (360-358.0f) / PTM_RATIO);
    verts[9].Set((640+635.0f) / PTM_RATIO, (360-359.0f) / PTM_RATIO);
...
{
    b2Vec2 tcPs = tieChuiBody->GetPosition();
    b2Vec2 p[4];
//    p[0] = b2Vec2((640.f+167.0f) / PTM_RATIO, (360.f+78.0f) / PTM_RATIO);
//    p[1] = b2Vec2((640.f+237.0f) / PTM_RATIO, (360.f+73.0f) / PTM_RATIO);
//    p[2] = b2Vec2((640.f+188.0f) / PTM_RATIO, (360.f+7.0f) / PTM_RATIO);
//    p[3] = b2Vec2((640.f+147.0f) / PTM_RATIO, (360.f+43.0f) / PTM_RATIO);
    
    p[0] = b2Vec2((640.f+106.0f) / PTM_RATIO, (360.f+2.0f) / PTM_RATIO);
    p[1] = b2Vec2((640.f+199.0f) / PTM_RATIO, (360.f-21.0f) / PTM_RATIO);
    p[2] = b2Vec2((640.f+222.0f) / PTM_RATIO, (360.f+74.0f) / PTM_RATIO);
    p[3] = b2Vec2((640.f+116.0f) / PTM_RATIO, (360.f+82.0f) / PTM_RATIO);
    
//    verts[14].Set(106.0f / PTM_RATIO, 2.0f / PTM_RATIO);
//    verts[15].Set(199.0f / PTM_RATIO, -21.0f / PTM_RATIO);
//    verts[16].Set(222.0f / PTM_RATIO, 74.0f / PTM_RATIO);
//    verts[17].Set(116.0f / PTM_RATIO, 82.0f / PTM_RATIO);
    
    
    bool isIn = PtInPolygon(tcPs,p,4);
    if(isIn == true)//张开
    {
        if(tooth1->GetPosition().y > 8.f/PTM_RATIO)
        {
            tooth1->SetTransform(b2Vec2(tooth1->GetPosition().x,tooth1->GetPosition().y-0.01f), tooth1->GetAngle());
        }
        if(tooth2->GetPosition().y < 218.f/PTM_RATIO)
        {
            tooth2->SetTransform(b2Vec2(tooth2->GetPosition().x,tooth2->GetPosition().y+0.01f), tooth1->GetAngle());
        }
    }
    else//闭合
    {
        if(tooth1->GetPosition().y < 60.f/PTM_RATIO)
        {
            tooth1->SetTransform(b2Vec2(tooth1->GetPosition().x,tooth1->GetPosition().y+0.01f), tooth1->GetAngle());
        }
        if(tooth2->GetPosition().y > 166.f/PTM_RATIO)
        {
            tooth2->SetTransform(b2Vec2(tooth2->GetPosition().x,tooth2->GetPosition().y-0.01f), tooth1->GetAngle());
        }
    }
