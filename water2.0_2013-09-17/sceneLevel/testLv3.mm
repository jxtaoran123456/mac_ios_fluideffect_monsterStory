...
-(id)init
{
    if (self = [super init])
    {
        ...
        backgroundSprite = [CCSprite spriteWithFile:@"lv3_back.png"];//testLevel1.png testLevel3
...
//    [self addSingleMap];
    ...
    // Particles
    //initFluidPraticles(1.2f,0.022f,420.f,30.f,3000,@"fire_010.png",nil);// VbmyR.png fire2 drop2
    initFluidPraticles(0.8f,0.012f,100.f,30.f,3000,@"fire_010.png",nil);// VbmyR.png fire2 drop2
...
-(int)winJudge
{
    ...
    for(int i = 0; i < nParticles; ++i)
	{
        if(PtInPoly(liquid[i].mPosition,poly,4) >= 0)
        {
            innerWinCount++;
        }
    }
    if(innerWinCount >= 3000 && result != 1)
    {
        NSLog(@"123");
        [self computeScore];
        result = 1;
