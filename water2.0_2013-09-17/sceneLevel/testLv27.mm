...
-(void)createStaticGeometry
{
    ...
    // Particles
    initFluidPraticles(0.8f,0.005f,100.f,30.f,2000,@"fire_010.png",nil);// VbmyR.png fire2 drop2
    ...
    bodyDef.position.Set((430.f)/PTM_RATIO, (220.f)/PTM_RATIO);
    bodyDef.angle = 0.f;
    b2Body *body = m_world->CreateBody(&bodyDef);
    // Define another box shape for our dynamic body.
    b2PolygonShape dynamicBox;
    dynamicBox.SetAsBox(0.55f,0.55f);
...
-(void)createRev
{
    b2BodyDef bodyDef;
    bodyDef.type = b2_staticBody;
    bodyDef.position.Set((715.f)/PTM_RATIO, (630.f)/PTM_RATIO);
    bodyDef.angle = 0.f;
    b2Body *body = m_world->CreateBody(&bodyDef);
    // Define another box shape for our dynamic body.
    b2PolygonShape dynamicBox;
    ...
    bodyDef2.position.Set((850.f)/PTM_RATIO, (665.f)/PTM_RATIO);
    bodyDef2.angle = 10.f* PI/180.f;;
    b2Body *body2 = m_world->CreateBody(&bodyDef2);
    // Define another box shape for our dynamic body.
    b2PolygonShape dynamicBox2;
    dynamicBox2.SetAsBox(4.5f,0.25f);
    // Define the dynamic body fixture.
    ...
    revoluteJointDef.localAnchorA.Set(0.0f,0.f);//the regular position for chain link joints, as above
    revoluteJointDef.localAnchorB.Set(-4.5f,0.f);//a little in from the edge of the circle
    
    revoluteJointDef.collideConnected = false;
    revoluteJointDef.enableMotor = false;
    revoluteJointDef.maxMotorTorque = -105000;
    revoluteJointDef.motorSpeed =  0.f * PI/180.f;//90 degrees per second
    revoluteJointDef.referenceAngle = 0;
    
    revoluteJointDef.enableLimit = false;
    revoluteJointDef.lowerAngle = -720.f * PI/180.f;// * PI/180.f;
    revoluteJointDef.upperAngle =  720.f * PI/180.f;// * PI/180.f;
...
//    verts[10].Set((640-630.0f) / PTM_RATIO, (360-355.0f) / PTM_RATIO);
    ...
    verts[0].Set((640+139.0f) / PTM_RATIO, (360+12.0f) / PTM_RATIO);
    verts[1].Set((640+46.0f) / PTM_RATIO, (360+263.0f) / PTM_RATIO);
    verts[2].Set((640+108.0f) / PTM_RATIO, (360+272.0f) / PTM_RATIO);
    verts[3].Set((640+156.0f) / PTM_RATIO, (360+47.0f) / PTM_RATIO);
    verts[4].Set((640+317.0f) / PTM_RATIO, (360+46.0f) / PTM_RATIO);
    verts[5].Set((640+347.0f) / PTM_RATIO, (360+319.0f) / PTM_RATIO);
    verts[6].Set((640+463.0f) / PTM_RATIO, (360+320.0f) / PTM_RATIO);
    verts[7].Set((640+535.0f) / PTM_RATIO, (360-356.0f) / PTM_RATIO);
    verts[8].Set((640-612.0f) / PTM_RATIO, (360-350.0f) / PTM_RATIO);
    verts[9].Set((640-609.0f) / PTM_RATIO, (360-287.0f) / PTM_RATIO);
    verts[10].Set((640-444.0f) / PTM_RATIO, (360-293.0f) / PTM_RATIO);
    verts[11].Set((640-442.0f) / PTM_RATIO, (360-55.0f) / PTM_RATIO);
    verts[12].Set((640-381.0f) / PTM_RATIO, (360-1.0f) / PTM_RATIO);
    verts[13].Set((640-266.0f) / PTM_RATIO, (360-2.0f) / PTM_RATIO);
    verts[14].Set((640-226.0f) / PTM_RATIO, (360-36.0f) / PTM_RATIO);
    verts[15].Set((640-227.0f) / PTM_RATIO, (360-294.0f) / PTM_RATIO);
    verts[16].Set((640+183.0f) / PTM_RATIO, (360-287.0f) / PTM_RATIO);
...
{
    if(body->toolsType == 1014 && zhuanZhou != NULL && tiaobanCount <= 2)
    {
        tiaobanCount++;
        //zhuanZhou->ApplyForce(b2Vec2(0.f,150000.f), b2Vec2(zhuanZhou->GetPosition().x,zhuanZhou->GetPosition().y));
        zhuanZhou->ApplyLinearImpulse(b2Vec2(0.f,200.9f), b2Vec2(zhuanZhou->GetPosition().x+2.0f,zhuanZhou->GetPosition().y));
    }
