...
-(void)createStaticGeometry
{
    ...
    // Particles
    initFluidPraticles(0.8f,0.002f,20.f,30.f,2000,@"fire_010.png",nil);// VbmyR.png fire2 drop2
    ...
    bodyDef.position.Set((780.f)/PTM_RATIO, (605.f)/PTM_RATIO);
    bodyDef.angle = 0.f;
    ...
    bodyDef2.position.Set((1120.f)/PTM_RATIO, (275.f)/PTM_RATIO);
    bodyDef2.angle = 0.f;
    b2Body *body2 = m_world->CreateBody(&bodyDef2);
    // Define another box shape for our dynamic body.
    b2PolygonShape dynamicBox2;
    dynamicBox2.SetAsBox(0.55f,5.15f);
    // Define the dynamic body fixture.
...
//    verts[10].Set((640-630.0f) / PTM_RATIO, (360-355.0f) / PTM_RATIO);
    ...
    verts[0].Set((640-522.6f) / PTM_RATIO, (360+128.0f) / PTM_RATIO);
    verts[1].Set((640+64.3f) / PTM_RATIO, (360+126.6f) / PTM_RATIO);
    verts[2].Set((640+68.6f) / PTM_RATIO, (360+242.5f) / PTM_RATIO);
    verts[3].Set((640+260.9f) / PTM_RATIO, (360+246.8f) / PTM_RATIO);
    verts[4].Set((640+268.0f) / PTM_RATIO, (360+122.3f) / PTM_RATIO);
    verts[5].Set((640+485.8f) / PTM_RATIO, (360+112.4f) / PTM_RATIO);
    verts[6].Set((640+426.4f) / PTM_RATIO, (360+0.7f) / PTM_RATIO);
    verts[7].Set((640+228.4f) / PTM_RATIO, (360-12.0f) / PTM_RATIO);
    verts[8].Set((640+238.3f) / PTM_RATIO, (360-236.9f) / PTM_RATIO);
    verts[9].Set((640+474.5f) / PTM_RATIO, (360-227.0f) / PTM_RATIO);
    verts[10].Set((640+603.2f) / PTM_RATIO, (360) / PTM_RATIO);//-191.6f
    verts[11].Set((640+586.2f) / PTM_RATIO, (360-323.1f) / PTM_RATIO);
    verts[12].Set((640-509.8f) / PTM_RATIO, (360-340.1f) / PTM_RATIO);
...
{
    if(body->toolsType == 1014 && gedang != NULL && gedang->GetPosition().y < 18.5f)
    {
